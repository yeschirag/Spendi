...
export const CategoryModal = ({ onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-md glass-card rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh] animate-fade-rise">
        ...
        <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-6 overflow-y-auto" style={{ fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" placeholder="e.g., Groceries" required value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-brand-black border border-border rounded-xl px-5 py-4 text-white placeholder:text-white/20 focus:outline-none focus:border-brand-porcelain transition-colors" />
          </div>
              ...
              {Object.keys(availableIcons).map(iconName => {
                ...
                return (
                  <button 
                    ...
                    className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all ${selectedIcon === iconName ? 'bg-brand-porcelain text-brand-black' : 'bg-brand-graphite/40 text-white hover:bg-brand-graphite/80 border border-border/40'}`}
                  >
            ...
            <div className="flex flex-wrap gap-3">
              {availableColors.map(color => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full transition-all ${selectedColor === color ? 'ring-2 ring-brand-porcelain ring-offset-2 ring-offset-black scale-110' : 'hover:scale-110'}`}
